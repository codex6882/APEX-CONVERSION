...
const useAdminAuth = () => {
  ...
  useEffect(() => {
    ...
    if ((Date.now() - parseInt(loginTime)) / 60000 > 30) { sessionStorage.clear(); navigate('/admin/login') }
  }, [navigate])
}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              <input type={showPass?'text':'password'} value={form.password} onChange={e=>setForm(p=>({...p,password:e.target.value}))} autoComplete="new-password"
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        {[
          ...
          <Link key={label} to={to} className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:border-blue-200 transition-colors card-hover">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {[
            { to:'/admin/kyc', label:'Review KYC Submissions', sub:`${stats.pendingKyc} pending reviews`, icon:ShieldCheck, color:'text-yellow-500' },
            { to:'/admin/withdrawals', label:'Process Withdrawals', sub:`${stats.pendingWithdrawals} pending requests`, icon:ArrowUpRight, color:'text-orange-500' },
            { to:'/admin/deposits', label:'Review Deposits', sub:`${stats.pendingDeposits} pending`, icon:Wallet, color:'text-blue-500' },
            { to:'/admin/users', label:'Manage Users', sub:`${stats.users} total users`, icon:Users, color:'text-green-500' },
...
const AdminUsers = () => {
  ...
  const sendSwift = async () => {
    if (!selected||!swiftCode) return; setSaving(true)
    await supabase.from('profiles').update({referral_code:`SWIFT:${swiftCode}`}).eq('id',selected.id)
    showMsg(`✅ Swift code sent to ${selected.username}!`); setSwiftCode(''); setSaving(false)
  }
    ...
    <div className="space-y-6">
      ...
      {selected ? (
        <div className="space-y-4">
          <button onClick={()=>setSelected(null)} className="text-blue-600 text-sm font-body flex items-center gap-1 hover:text-blue-800">← Back to users</button>
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
            <div className="flex items-center gap-4 mb-6 pb-4 border-b border-gray-100">
              <div className="w-14 h-14 bg-blue-600 rounded-full flex items-center justify-center text-white text-xl font-heading font-bold">
                {selected.username?.[0]?.toUpperCase()}
              </div>
              <div>
                <p className="font-heading font-bold text-navy-900 text-lg">{selected.full_name}</p>
                <p className="text-gray-500 text-sm font-body">@{selected.username}</p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3 mb-6">
              {[['Email',selected.email],['Phone',selected.phone],['Country',selected.country],['KYC',selected.kyc_status],['Balance',`$${Number(selected.balance).toFixed(2)}`],['Bonus',`$${Number(selected.bonus).toFixed(2)}`],['Joined',new Date(selected.created_at).toLocaleDateString()]].map(([k,v])=>(
                <div key={k} className="bg-gray-50 rounded-xl p-3">
                  ...
                  <p className="text-navy-900 text-sm font-body font-semibold">{v}</p>
                </div>
              ))}
            </div>
            <div className="space-y-4 border-t border-gray-100 pt-4">
              <div>
                ...
                <div className="flex gap-2">
                  <input type="number" value={editBalance} onChange={e=>setEditBalance(e.target.value)} placeholder="Enter new balance"
                    className="flex-1 border border-gray-200 rounded-xl px-3 py-2 text-sm font-body focus:outline-none focus:border-blue-500"/>
                  <button onClick={saveBalance} disabled={saving} className="bg-blue-600 text-white px-4 py-2 rounded-xl text-sm font-body font-bold hover:bg-blue-700 transition-colors">Save</button>
                </div>
              </div>
              <div>
                <p className="font-body font-bold text-navy-900 text-sm mb-2">Add Balance / Profit</p>
                <div className="flex gap-2">
                  <input type="number" value={addAmount} onChange={e=>setAddAmount(e.target.value)} placeholder="Amount to add"
                    className="flex-1 border border-gray-200 rounded-xl px-3 py-2 text-sm font-body focus:outline-none focus:border-blue-500"/>
                  <button onClick={addProfit} disabled={saving} className="bg-green-600 text-white px-4 py-2 rounded-xl text-sm font-body font-bold hover:bg-green-700 transition-colors">Add</button>
                </div>
              </div>
              <div>
                <p className="font-body font-bold text-navy-900 text-sm mb-2">Send Swift Code</p>
                <div className="flex gap-2">
                  <input type="text" value={swiftCode} onChange={e=>setSwiftCode(e.target.value)} placeholder="Enter Swift Code"
                    className="flex-1 border border-gray-200 rounded-xl px-3 py-2 text-sm font-body focus:outline-none focus:border-blue-500"/>
                  <button onClick={sendSwift} disabled={saving} className="bg-navy-900 text-white px-4 py-2 rounded-xl text-sm font-body font-bold hover:bg-navy-800 transition-colors">Send</button>
                </div>
              </div>
              {selected.kyc_status !== 'verified' && (
                <button onClick={verifyKyc} disabled={saving} className="w-full bg-yellow-500 hover:bg-yellow-600 text-white py-2 rounded-xl text-sm font-body font-bold transition-colors flex items-center justify-center gap-2">
          ...
          {users.length===0 ? <p className="text-gray-400 font-body text-sm">No users yet.</p> :
            users.map(u=>(
              <div key={u.id} className="bg-white rounded-xl p-4 border border-gray-100 flex items-center justify-between shadow-sm">
                ...
                <button onClick={()=>{setSelected(u);setEditBalance(String(u.balance))}} className="bg-blue-50 text-blue-600 px-3 py-2 rounded-xl text-xs font-body font-bold flex items-center gap-1 hover:bg-blue-100 transition-colors">
                  <Eye size={14}/> Manage
                </button>
              </div>
            ))}
...
const AdminDeposits = () => {
  ...
  useEffect(()=>{
    supabase.from('deposits').select('*, profiles(username,email)').order('created_at',{ascending:false}).then(({data})=>{ if(data) setDeposits(data) })
  },[])
          ...
          <div key={d.id} className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-gray-400 text-xs font-body">{d.profiles?.email} · {new Date(d.created_at).toLocaleDateString()}</p>
              </div>
            ...
            {d.proof_url && (
              <div>
                <p className="text-xs font-body text-gray-400 mb-2">Payment Proof:</p>
                <img src={getUrl(d.proof_url)} alt="Proof" className="w-full max-h-48 object-contain rounded-xl border border-gray-100"/>
              </div>
            )}
            {d.status==='pending' && (
              <div className="flex gap-3">
                <button onClick={()=>approve(d)} className="flex-1 bg-green-600 text-white py-2 rounded-xl text-sm font-body font-bold flex items-center justify-center gap-1 hover:bg-green-700 transition-colors"><CheckCircle size={14}/> Approve & Credit</button>
                <button onClick={()=>reject(d.id)} className="flex-1 bg-red-50 text-red-600 py-2 rounded-xl text-sm font-body font-bold flex items-center justify-center gap-1 hover:bg-red-100 transition-colors"><XCircle size={14}/> Reject</button>
              </div>
            )}
          </div>
        ))}
...
const AdminKYC = () => {
  ...
  useEffect(()=>{
    supabase.from('kyc_submissions').select('*, profiles(username,email)').eq('status','pending').order('created_at',{ascending:false}).then(({data})=>{ if(data) setSubmissions(data) })
  },[])
      ...
      {submissions.length===0 ? <p className="text-gray-400 font-body text-sm">No pending KYC submissions.</p> :
        submissions.map(sub=>(
          <div key={sub.id} className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="font-heading font-bold text-navy-900">@{sub.profiles?.username}</p>
                <p className="text-gray-400 text-xs font-body">{sub.profiles?.email} · {sub.document_type?.replace('_',' ')}</p>
              </div>
              <span className="bg-yellow-100 text-yellow-700 text-xs px-2 py-1 rounded-full font-body font-medium">Pending</span>
            </div>
            <div className="grid grid-cols-2 gap-3">
              {[['Front',sub.front_url],['Back',sub.back_url]].map(([label,url])=>(
                <div key={label}>
                  <p className="text-xs font-body text-gray-400 mb-1">{label}</p>
                  <img src={getUrl(url)} alt={label} className="w-full h-32 object-cover rounded-xl border border-gray-100"/>
                </div>
              ))}
            </div>
            <div className="flex gap-3">
              <button onClick={()=>update(sub,'approved')} className="flex-1 bg-green-600 text-white py-2 rounded-xl text-sm font-body font-bold flex items-center justify-center gap-1 hover:bg-green-700 transition-colors"><CheckCircle size={14}/> Approve</button>
              <button onClick={()=>update(sub,'rejected')} className="flex-1 bg-red-50 text-red-600 py-2 rounded-xl text-sm font-body font-bold flex items-center justify-center gap-1 hover:bg-red-100 transition-colors"><XCircle size={14}/> Reject</button>
            </div>
          </div>
        ))}
    </div>
...
const AdminWithdrawals = () => {
  ...
  useEffect(()=>{
    supabase.from('withdrawals').select('*, profiles(username)').order('created_at',{ascending:false}).then(({data})=>{ if(data) setWithdrawals(data) })
  },[])
      ...
      {withdrawals.length===0 ? <p className="text-gray-400 font-body text-sm">No withdrawals yet.</p> :
        withdrawals.map(w=>(
          <div key={w.id} className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="font-heading font-bold text-navy-900">@{w.profiles?.username}</p>
            ...
            <div className="bg-gray-50 rounded-xl p-3 space-y-2">
              <div className="flex justify-between"><span className="text-gray-400 text-xs font-body">Wallet Address</span><span className="text-navy-900 text-xs font-body font-semibold break-all max-w-xs text-right">{w.btc_address}</span></div>
              <div className="flex justify-between"><span className="text-gray-400 text-xs font-body">Swift Code</span><span className="text-navy-900 text-xs font-body font-semibold">{w.swift_code}</span></div>
            </div>
            {w.status==='pending' && (
              <div className="flex gap-3">
                <button onClick={()=>update(w.id,'approved')} className="flex-1 bg-green-600 text-white py-2 rounded-xl text-sm font-body font-bold flex items-center justify-center gap-1 hover:bg-green-700 transition-colors"><CheckCircle size={14}/> Approve</button>
                <button onClick={()=>update(w.id,'rejected')} className="flex-1 bg-red-50 text-red-600 py-2 rounded-xl text-sm font-body font-bold flex items-center justify-center gap-1 hover:bg-red-100 transition-colors"><XCircle size={14}/> Reject</button>
              </div>
            )}
          </div>
        ))}
...
const AdminDashboard = () => {
  ...
  const logout = () => { sessionStorage.clear(); navigate('/admin/login') }
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <aside className="hidden md:flex flex-col w-56 bg-navy-900 fixed top-0 left-0 bottom-0 z-50">
        <div className="p-4 border-b border-white/10">
          <ApexLogo white/>
          <p className="text-white/40 text-xs font-body mt-1">Admin</p>
        </div>
        ...
        <div className="p-3 border-t border-white/10">
          <p className="text-white/30 text-xs font-body px-3 mb-2">Session: 30 min remaining</p>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 bg-navy-900 z-50 h-14 flex items-center justify-between px-4">
        <ApexLogo white/>
        <button onClick={logout} className="text-white/60"><LogOut size={18}/></button>
      </div>
